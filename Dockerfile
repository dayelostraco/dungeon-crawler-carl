...
# App code
COPY server.py config.py generator.py voice.py archive.py display.py player.py main.py ./
COPY static/ static/
...
HEALTHCHECK --interval=30s --timeout=5s --retries=3 \
    CMD python -c "import urllib.request; urllib.request.urlopen('http://localhost:8000/api/achievements')"
